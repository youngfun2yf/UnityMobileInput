...
namespace UnityEngine.UI
{
    ...
    [AddComponentMenu("UI/Custom Input Field", 31)]
        ...
        ILayoutElement
    {
        ...
        public delegate void OnDelete(string text, ref int startIndex, ref int endIndex);

        public delegate void OnInsert(ref string text, ref int startIndex, int length);
        ...
        private OnDelete m_OnDelete;

        private OnInsert m_OnInsert;
        ...
        public OnDelete onDelete {
            ...
            set { SetPropertyUtility.SetClass(ref m_OnDelete, value); }
        }

        public OnInsert onInsert {
            get => m_OnInsert;
            set => SetPropertyUtility.SetClass(ref m_OnInsert, value);
        }
        ...
        private void Delete()
        {
            ...
            if (caretPositionInternal < caretSelectPositionInternal)
            {
                if (onDelete != null)
                {
                    int start = caretPositionInternal;
                    int end = caretSelectPositionInternal;
                    onDelete(m_Text, ref start, ref end);
            ...
            else
            {
                if (onDelete != null)
                {
                    int start = caretSelectPositionInternal;
                    int end = caretPositionInternal;
                    onDelete(m_Text, ref start, ref end);
                    caretSelectPositionInternal = start;
                    caretPositionInternal = end;
                }
        ...
        private void Backspace()
        {
            ...
            else
            {
                ...
                {
                    int start = caretPositionInternal - 1;
                    int end = caretPositionInternal;
                    if (onDelete != null)
                    {
                        onDelete(m_Text, ref start, ref end);
                        caretPositionInternal = start + 1;
                    }
                    m_Text = text.Remove(start, end - start);
        ...
        private void Insert(char c)
        {
            ...
            if (onInsert != null)
            {
                ...
                onInsert(ref m_Text, ref start, replaceString.Length);
